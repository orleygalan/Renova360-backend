...
class Comentario_servicio_molde
{
    ...
    public function agregar_comentario($usuario_ID, $servicio_ID, $comentario)
    {
        try {
            $query = 'INSERT INTO comentarios_servicios 
                                (usuario_ID, servicio_ID, comentario) 
                    VALUES (:usuario_ID, servicio_ID, :comentario)';
    ...
    public function eliminar_comentario($comentario_ID, $usuario_ID, $es_admin = false)
    {
        try {
            if ($es_admin) {
                ...
                $query = 'DELETE FROM comentarios_servicios WHERE comentario_ID = :comentario_ID';
                $stmt = $this->conn->prepare($query);
                $stmt->bindParam(':comentario_ID', $comentario_ID);
            } else {
                //  El usuario solo puede eliminar sus propios comentarios
                $query = 'DELETE FROM comentarios_servicios 
